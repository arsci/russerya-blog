...
It was a beautiful, sunny, Thursday afternoon. I had just returned from lunch and was getting ready to head to 
...
for the airport the next day. I'd finally be able to get rid of a few POE injectors scattered around 
...
relies on (the unfortunate story of many successful POCs). It now runs Home Assistant and ZWaveJS for all of our home automation and controls, and PiHole for DNS/ad-blocking. It has zero redundancy, but does have automated backups for the critical services so if something does happen, I can at least cobble it back 
together with minimal downtime. 

This old laptop server would also need to be updated, along with Home Assistant, ZWaveJS, and PiHole.
...
immediately jumped to the flashing cursor. It wouldn't accept any keyboard inputs, even from a secondary USB keyboard this time. Still no logs or any other indication Ubuntu was doing _anything_. 
...
At this point I had to decide what to do next. The Ubiquiti switch had arrived by this point, but I didn't want to further risk the network being down while I was gone, so I 
...
Once it was installed, I set up docker and brought up Home Assistant, ZWaveJS, and PiHole, all set to the latest stable images. All three services started up normally from blank configs. Now it was time 
to try and restore the backups. 

Since Home Assistant and ZWaveJS are docker containers, they do not have a restore option from their respective GUIs. I had to manually copy the 
